...
void vvc_intra_pred_chroma(const OVCTUDec *const lc_ctx,
    ...
    uint8_t neighbour= lc_ctx->ctu_ngh_flags;
    uint8_t got_left_ctu = neighbour & VVC_CTU_LEFT_FLAG;
    uint8_t got_top_ctu  = neighbour & VVC_CTU_UP_FLAG;
    // FIXED? : const uint16_t *const src_cb = &lc_ctx->ctu_data_cb[VVC_CTB_OFFSET];
    // FIXED? : const uint16_t *const src_cr = &lc_ctx->ctu_data_cr[VVC_CTB_OFFSET];
    const uint16_t *const src_cb = &lc_ctx->rcn_ctx.rcn_ctu_buff.data_cb[VVC_CTB_OFFSET];
    const uint16_t *const src_cr = &lc_ctx->rcn_ctx.rcn_ctu_buff.data_cr[VVC_CTB_OFFSET];
    // FIXED? : uint64_t left_col_map = lc_ctx->progress_map_c.cols[x0 >> 1];
    // FIXED? : uint64_t top_row_map  = lc_ctx->progress_map_c.rows[y0 >> 1];
    uint64_t left_col_map = lc_ctx->rcn_ctx.progress_field.vfield[x0 >> 1];
    uint64_t top_row_map  = lc_ctx->rcn_ctx.progress_field.hfield[y0 >> 1];
    ...
    case VVC_LM_CHROMA:
    {
        // FIXED? : const uint16_t  *const src_luma = &lc_ctx->ctu_data_y[VVC_CTB_OFFSET+(x0<<1)+((y0<<1)*VVC_CTB_STRIDE)];
        const uint16_t  *const src_luma = &lc_ctx->rcn_ctx.rcn_ctu_buff.data_y[VVC_CTB_OFFSET+(x0<<1)+((y0<<1)*VVC_CTB_STRIDE)];

        // lc_ctx->cclm_func(src_luma, dst_cb, dst_cr, log2_pb_width, log2_pb_height,
    ...
    case VVC_MDLM_LEFT:
    {
      ...
      const uint16_t  *const src_luma = &lc_ctx->rcn_ctx.rcn_ctu_buff.data_y[VVC_CTB_OFFSET+(x0<<1)+((y0<<1)*VVC_CTB_STRIDE)];
        ...
        // FIXED? : lc_ctx->left_mdlm(src_luma, dst_cb, dst_cr,
    ...
    case VVC_MDLM_TOP:
    {
      // FIXED? : const uint16_t  *const src_luma = &lc_ctx->ctu_data_y[VVC_CTB_OFFSET+(x0<<1)+((y0<<1)*VVC_CTB_STRIDE)];
        ...
        // FIXED? : lc_ctx->top_mdlm(src_luma, dst_cb, dst_cr,
...
void vvc_intra_pred(const OVCTUDec *const lc_ctx,
                    uint16_t *const src,
                    ptrdiff_t dst_stride,
                    uint8_t intra_mode, int x0, int y0,
                    int log2_pb_width, int log2_pb_height){
    ...
    //                 lc_ctx->progress_map.cols[x0 >> 2],
    //         lc_ctx->progress_map.rows[y0 >> 2],
    //         x0, y0, log2_pb_width, log2_pb_height,0);

            fill_ref_left_0(src,dst_stride,ref2,
                            lc_ctx->rcn_ctx.progress_field.vfield[x0 >> 2],
                    lc_ctx->rcn_ctx.progress_field.hfield[y0 >> 2],
                    x0, y0, log2_pb_width, log2_pb_height,0);
...
void vvc_intra_pred_isp(const OVCTUDec *const lc_ctx,
                        uint16_t *const src,
                        ptrdiff_t dst_stride,
                        uint8_t intra_mode,
                        int x0, int y0,
    ...
    // FIXED? :fill_ref_left_0(src, dst_stride, ref2,
    //                 lc_ctx->progress_map.cols[(x0 >> 2) + !!(offset_x % 4)],
    //                 lc_ctx->progress_map.rows[((y0 ) >> 2) + !!(y0 % 4)],
    //                 x0, y0 - offset_y, log2_cu_width, log2_cu_height, offset_y);
    fill_ref_left_0(src, dst_stride, ref2,
                    lc_ctx->rcn_ctx.progress_field.vfield[(x0 >> 2) + !!(offset_x % 4)],
                    lc_ctx->rcn_ctx.progress_field.hfield[((y0 ) >> 2) + !!(y0 % 4)],
                    x0, y0 - offset_y, log2_cu_width, log2_cu_height, offset_y);
    //  FIXED? :fill_ref_above_0(src, dst_stride, ref1,
    //                  lc_ctx->progress_map.rows[(y0 >> 2) + !!(offset_y % 4)],
    //                  lc_ctx->progress_map.cols[((x0 ) >> 2) + !!(x0 % 4)],
    //                  x0 - offset_x, y0, log2_cu_width, log2_cu_height, offset_x);
   fill_ref_above_0(src, dst_stride, ref1,
                    lc_ctx->rcn_ctx.progress_field.hfield[(y0 >> 2) + !!(offset_y % 4)],
                    lc_ctx->rcn_ctx.progress_field.vfield[((x0 ) >> 2) + !!(x0 % 4)],
                    x0 - offset_x, y0, log2_cu_width, log2_cu_height, offset_x);
    ref1+= offset_x;
    ref2+= offset_y;
    for (int i = 0; i < 4; ++i){
...
void vvc_intra_pred_multi_ref( const OVCTUDec *const lc_ctx,
                               uint16_t *const src,
                               ptrdiff_t dst_stride,
                               uint8_t intra_mode, int x0, int y0,
                               int log2_pb_width, int log2_pb_height,
...
void
vvc_intra_pred_mip(const OVCTUDec *const lc_ctx,
                   uint16_t *const dst,
                   int x0, int y0, int log2_pu_w, int log2_pu_h,
                   uint8_t mip_mode)
{
  // FIXED? :const uint16_t *src = &lc_ctx->ctu_data_y[VVC_CTB_OFFSET];
  const uint16_t *src = &lc_ctx->rcn_ctx.rcn_ctu_buff.data_y[VVC_CTB_OFFSET];
...
void
vvc_intra_pred_mip_tr(const OVCTUDec *const lc_ctx,
                      uint16_t *const dst,
                      int x0, int y0, int log2_pu_w, int log2_pu_h,
                      uint8_t mip_mode)
{
