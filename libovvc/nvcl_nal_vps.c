...
typedef struct OVVPS
{
    ...
    uint8_t vps_max_tid_ref_present_flag[0];
    uint8_t vps_direct_ref_layer_flag[0][0];
    uint8_t vps_max_tid_il_ref_pics_plus1[0][0];
    uint8_t vps_each_layer_is_an_ols_flag;
    uint8_t vps_ols_mode_idc;
    uint8_t vps_num_output_layer_sets_minus2;
    uint8_t vps_ols_output_layer_flag[0][0];
    uint8_t vps_num_ptls_minus1;
    uint8_t vps_pt_present_flag[0];
    uint8_t vps_ptl_max_tid[0];
    uint8_t vps_ptl_alignment_zero_bit;
    OVPTL * profile_tier_level;
    uint8_t vps_ols_ptl_idx[0];
    uint8_t vps_num_dpb_params_minus1;
    uint8_t vps_sublayer_dpb_params_present_flag;
    uint8_t vps_dpb_max_tid[0];
    OVDPBParams *dpb_params;
    uint8_t vps_ols_dpb_pic_width[0];
    uint8_t vps_ols_dpb_pic_height[0];
    uint8_t vps_ols_dpb_chroma_format[0];
    uint8_t vps_ols_dpb_bitdepth_minus8[0];
    uint8_t vps_ols_dpb_params_idx[0];
    ...
    uint8_t vps_hrd_max_tid[0];
    OVOLSHRDTiming *ols_hrd;
    uint8_t vps_ols_timing_hrd_idx[0];
    /* unused */
                  ...
                  OVNVCLCtx *const nvcl_ctx)
{
    ...
    if (vps->vps_max_layers_minus1 > 0) {
        ...
        if (!vps->vps_each_layer_is_an_ols_flag) {
            ...
            if (vps->vps_ols_mode_idc == 2) {
                ...
                for (uint8_t i = 1; i <= vps->vps_num_output_layer_sets_minus2 + 1; i ++) {
                    for (uint8_t j = 0; j <= vps->vps_max_layers_minus1; j++) {
    ...
    for (uint8_t i = 0; i <= vps->vps_num_ptls_minus1; i++) {
        if (i > 0) {
    ...
    if (!vps->vps_each_layer_is_an_ols_flag) {
        ...
        if (vps->vps_timing_hrd_params_present_flag) {
            ...
            for (uint8_t i = 0; i <= vps->vps_num_ols_timing_hrd_params_minus1; i++) {
                if (!vps->vps_default_ptl_dpb_hrd_max_tid_flag) {
                    vps->vps_hrd_max_tid[i] = nvcl_read_flag(rdr);
                }
