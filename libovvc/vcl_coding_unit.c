...
inter_affine_mvp_data_p(OVCTUDec *const ctu_dec, uint8_t nb_active_ref_min1)
{
    ...
    mvp_data.tmp_affine_type = affine_type;
                        ...
                        uint8_t skip_flag, uint8_t merge_flag)
{
    ...
    if (merge_flag) {
        ...
        mmvd_flag = mrg_data.merge_type == P_MMVD_MERGE;
        ...
        } else if (mrg_data.merge_type == P_GPM_MERGE) {
            uint8_t max_nb_cand = ctu_dec->max_num_merge_candidates;
        ...
        } else if (mrg_data.merge_type == P_CIIP_MERGE) {
            uint8_t max_nb_cand = ctu_dec->max_num_merge_candidates;
        ...
        } else if (mrg_data.merge_type == P_MMVD_MERGE){
            uint8_t max_nb_cand = ctu_dec->max_num_merge_candidates;
        ...
        if (inter_dir == 0x3) {
            ...
            if (ctu_dec->affine_enabled && log2_cb_w > 3 && log2_cb_h > 3) {
                ...
                if (affine_flag) {
                    ...
                    struct AffineMVPDataP aff_mvp_data = inter_affine_mvp_data_p(ctu_dec, nb_active_ref_min1);
